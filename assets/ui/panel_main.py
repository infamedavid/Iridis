...
class IRIDIS_PT_main_panel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        protect_row = box.row(align=True)
        protect_row.prop(settings, "protect_mask_image", text="Protect Mask", icon='IMAGE_DATA')
        box.prop(settings, "protect_mask_image", text="Assigned", icon='IMAGE_DATA')
        ...
        rough_row = box.row(align=True)
        rough_row.prop(settings, "roughness_bias")
        rough_row.prop(settings, "roughness_control_mask_image", text="", icon='IMAGE_DATA')
        box.prop(settings, "roughness_control_mask_image", text="Assigned", icon='IMAGE_DATA')
        box.prop(settings, "microdetail_influence")
        box.prop(settings, "cavity_boost")

        box = layout.box()
        box.label(text="Metallic")
        metal_row = box.row(align=True)
        metal_row.prop(settings, "metallic_bias")
        metal_row.prop(settings, "metallic_control_mask_image", text="", icon='IMAGE_DATA')
        box.prop(settings, "metallic_control_mask_image", text="Assigned", icon='IMAGE_DATA')
        box.prop(settings, "paint_color_rejection")
        box.prop(settings, "threshold_softness")

        box = layout.box()
        box.label(text="Normal")
        normal_row = box.row(align=True)
        normal_row.prop(settings, "normal_strength")
        normal_row.prop(settings, "relief_control_mask_image", text="", icon='IMAGE_DATA')
        box.prop(settings, "relief_control_mask_image", text="Assigned", icon='IMAGE_DATA')
